...
Trigger.prototype.PersianAttackCav = function(data)
{
	...
	for (let i = 0; i < num_attackers; ++i)
	{
		...
		TriggerHelper.SpawnUnits(this.persianSpawnSites[this.temp_site],temp_name,1,2);
	}
...
Trigger.prototype.PersianAttack = function(data)
{
	...
	var all_units = TriggerHelper.GetEntitiesByPlayer(2);
	var pop_limit = 20+5*this.numberOfTimerTrigger;
	...
	{
		temp_name = this.persianInfTypes[Math.floor(Math.random() * Math.floor(this.persianInfTypes.length))];
		TriggerHelper.SpawnUnits(this.persianSpawnSites[site],temp_name,1,2);
	}
	...
	if (this.numberOfTimerTrigger > 7) 
	{
		...
		{
			temp_name = this.persianInfTypes[Math.floor(Math.random() * Math.floor(this.persianInfTypes.length))];
			TriggerHelper.SpawnUnits(this.persianSpawnSites[second_site],temp_name,1,2);
		}
...
Trigger.prototype.GreekAttack = function(data)
{
	...
	var pop_limit = 20+5*this.greekAttackCounter;
	if (all_units.length > pop_limit)
	{
...

{
	...
	cmpTrigger.cavRatio = 0.65;
	cmpTrigger.siegeRatio = 0.3;
